class FormbuilderModel extends Backbone.DeepModel
  ...
  is_form_setting_input: ->
    Formbuilder.inputFormSettingsFields[@get(Formbuilder.options.mappings.FIELD_TYPE)]?
...
class ViewFieldView extends Backbone.View
  ...
  initialize: (options) ->
    ...
    @model.attributes.field_options.form_label_placement = Formbuilder.formLabelPlacement
  render: ->  
    console.log("change triggered")  
    @$el.addClass('response-field-' + @model.get(Formbuilder.options.mappings.FIELD_TYPE))
        .data('cid', @model.cid)
        .html(Formbuilder.templates["view/base#{if @model.is_form_setting_input() then '_input_form_settings' else if !@model.is_input() then '_non_input' else ''}"]({rf: @model}))
...
class FormEditSettingsView extends Backbone.View
  className: "form-edit-settings-field"
  ...
  render: ->
    @$el.html(Formbuilder.templates["form_config/form_setting"]({rf: @model}))    
...
class EditFieldView extends Backbone.View
  ...
  addStatement: (e) ->
    $el = $(e.currentTarget)
    i = @$el.find('.statement').index($el.closest('.statement'))
    options = @model.get(Formbuilder.options.mappings.STATEMENTS) || []
    newOption = {label: ""}
    ...
    @parentView.not_include_remove = "false"
    @model.set Formbuilder.options.mappings.STATEMENTS, options
    @model.trigger "change:#{Formbuilder.options.mappings.STATEMENTS}"
    @forceRender()

  removeStatement: (e) ->
    $el = $(e.currentTarget)
    index = @$el.find(".js-remove-statement").index($el)
    options = @model.get Formbuilder.options.mappings.STATEMENTS
    options.splice index, 1
    @model.set Formbuilder.options.mappings.STATEMENTS, options
    @model.trigger "change:#{Formbuilder.options.mappings.STATEMENTS}"
...
class BuilderView extends Backbone.View
  ...
  addFormSettings : () ->
    ...
    rf = @collection.create Formbuilder.helpers.defaultFieldAttrs(field_type)
  ...
  createAndShowFormEditView: (model) ->
    @formEditView = new FormEditSettingsView
      model: model
      parentView: @

    $newFormEditEl = @formEditView.render().$el
    @$el.find(".fb-form-settings-wrapper").html $newFormEditEl

    return @

  createAndShowFormView: (model) ->
    @formView = new FormSettingsView
      model: model
      parentView: @

    $newFormEl = @formView.render().$el
    @$el.find(".fb-right").prepend $newFormEl
  ...
  handleFormUpdate: (responseField, _, options)->
    if responseField && responseField.attributes.field_type == 'form_header'
      @checkTemp @coll, responseField.attributes.field_options.form_label_placement for @coll in @collection.models
          
    return if @updatingBatch
    @formSaved = false
    #@saveFormButton.removeAttr('disabled').text(Formbuilder.options.dict.SAVE_FORM)

  checkTemp : (@coll, labelAlignment) ->
    if @coll.attributes.field_type != 'form_header'
      @coll.attributes.field_options.form_label_placement = labelAlignment
...
class Formbuilder
  ...
  @registerField: (name, opts) ->
    ...
    if opts.type == 'non_input'
      ...
      Formbuilder.inputFormSettingsFields[name] = opts
    else
